...
build:
	...
	docker build --build-arg BASE_IMAGE=$(BASE_IMAGE) -t confluence-demo-container:latest ./demo-container
...
define skill_test_docker_run
	docker run --rm \
		...
		$(TELEMETRY_ENV_VARS) \
		$(CLAUDE_AUTH_ENV) \
		-v $(CONFLUENCE_PLUGIN_PATH):/home/devuser/.claude/plugins/cache/confluence-assistant-skills/confluence-assistant-skills/dev:ro \
		-v $(CONFLUENCE_LIB_PATH):/opt/confluence-lib:ro \
		-v $(CONFLUENCE_DIST_PATH):/opt/confluence-dist:ro \
		...
		-c "pip install -q /opt/confluence-dist/*.whl 2>/dev/null; \
		    rm -f ~/.claude/plugins/cache/confluence-assistant-skills/confluence-assistant-skills/1.0.0 2>/dev/null; \
		    ln -sf dev ~/.claude/plugins/cache/confluence-assistant-skills/confluence-assistant-skills/1.0.0 2>/dev/null; \
		    ...
		    $(if $(FIX_CONTEXT),--fix-context $(CONFLUENCE_SKILLS_PATH),)"
endef
...
otel-reset:
	...
	docker volume rm confluence-demo_lgtm-data || true
...
# Usage: make run-scenario SCENARIO=page
#        make run-scenario SCENARIO=search DELAY=5
run-scenario:
	@if [ -z "$(SCENARIO)" ]; then echo "Usage: make run-scenario SCENARIO=<name> [DELAY=3]"; echo "Scenarios: page, search, space, template, hierarchy, comment, attachment, label, permission, bulk, analytics"; exit 1; fi
	$(call check_claude_auth)
...
test-skill-mock-dev:
	...
	$(call check_claude_auth)
	@mkdir -p $(CLAUDE_SESSIONS_DIR) $(CHECKPOINTS_DIR)
	@$(call skill_test_docker_run,-e CONFLUENCE_MOCK_MODE=true -e PYTHONPATH=/workspace/patches,-v $(PWD)/demo-container/patches:/workspace/patches:ro,--mock)
...
refine-skill:
	...
	python demo-container/skill-refine-loop.py \
		--scenario $(SCENARIO) \
		--confluence-skills-path $(CONFLUENCE_SKILLS_PATH) \
...
#        make refine-all-mocks SCENARIOS="search,page"
...
help:
	...
	@echo "  make reset-sandbox  - Reset Confluence sandbox to initial state"
	@echo "  make seed-sandbox   - Seed Confluence sandbox with demo data"
	@echo ""
	...
	@echo ""
	@echo "Testing:"
	@echo "  make run-scenario SCENARIO=page       - Run scenario in debug mode (auto-advance)"
	...
	@echo "  make test-skill-mock SCENARIO=search  - Run with mocked Confluence API (fast)"
