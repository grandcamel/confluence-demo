...
Usage:
    ...
    python skill-test.py scenarios/search.prompts --mock      # Use mocked Confluence API

Fast Iteration with Checkpoints:
    # Step 1: Run full scenario to create checkpoints
    python skill-test.py scenarios/page.prompts --checkpoint-file /tmp/page.json

    # Step 2: List available checkpoints
    python skill-test.py scenarios/page.prompts --checkpoint-file /tmp/page.json --list-checkpoints
...
JUDGE_PROMPT_TEMPLATE = """You are evaluating Claude's response quality for a Confluence automation skill test.
...
2. **Bash tool** executes the `confluence` CLI commands described in the skill

The pattern `['Skill', 'Bash']` is CORRECT and EXPECTED for any Confluence operation:
...
def generate_fix_context(result: PromptResult, skills_path: str) -> dict[str, Any]:
    ...
    plugin_path = Path(skills_path) / "plugins" / "confluence-assistant-skills"
    if not plugin_path.exists():
        plugin_path = Path(skills_path) / "confluence-assistant-skills"
    skills_dir = plugin_path / "skills"

    if skills_dir.exists():
        # Look for skill files that might be relevant
        # For Confluence queries, likely confluence-search or confluence-assistant
        relevant_skills = []
        prompt_lower = result.spec.prompt.lower()

        skill_keywords = {
            "confluence-search.md": ["search", "find", "query", "cql", "pages"],
            "confluence-page.md": ["create", "update", "page", "blog", "content"],
            "confluence-assistant.md": ["confluence", "assistant"],
            "confluence-space.md": ["space", "spaces", "create space"],
            "confluence-hierarchy.md": ["children", "parent", "tree", "ancestors"],
            "confluence-comment.md": ["comment", "comments", "reply"],
            "confluence-attachment.md": ["attachment", "upload", "file"],
            "confluence-label.md": ["label", "labels", "tag"],
            "confluence-template.md": ["template", "blueprint"],
            "confluence-permission.md": ["permission", "access", "restrict"],
            "confluence-analytics.md": ["view", "views", "watchers", "popular"],
            "confluence-bulk.md": ["bulk", "multiple", "batch"],
        }
    ...
    try:
        git_log = subprocess.run(
            ["git", "log", "--oneline", "-10", "--", "confluence-assistant-skills/skills/"],
            cwd=skills_path,
            capture_output=True,
            text=True,
            timeout=10,
...
Telemetry (enabled by default):
    ...
    parser.add_argument("--fix-context", type=str, metavar="SKILLS_PATH",
                        help="Output fix context JSON for first failure (requires path to Confluence-Assistant-Skills)")
    ...
    parser.add_argument("--mock", action="store_true",
                        help="Enable Confluence API mocking for faster, deterministic tests")
    ...
    if args.mock or os.environ.get("CONFLUENCE_MOCK_MODE", "").lower() == "true":
